...
public class SettingsPersonaliseYourSerActivity extends BaseActivity implements
	...
	private PersonItemInfo PersonItemInfo;
	...
	private void initdata() {
		...
		name.setText(UserInfo.getUserName() + "'s" + " phone");
		image1.setImageResource(Integer.valueOf(UserInfo.getHeadFaceImage()));
	...
	public void onClick(View v) {
		...
		case R.id.go_left:
			if (isDavid) {
				...
				name.setText(UserInfo.getUserName() + "'s" + " phone");
				edit_name.setText(UserInfo.getUserName() + "'s" + " phone");
				number.setText(UserInfo.getUserMobile().substring(0, 4)
						+ " "
		...
		case R.id.go_right:
			if (isDavid) {
				...
				number.setText(UserInfo.getUserMobile(1).substring(0, 4)
						+ " "
	...
	private void showPopuwindow(int resId) {
		...
		params.alpha = 0.6f;
		SettingsPersonaliseYourSerActivity.this.getWindow().addFlags(WindowManager.LayoutParams.FLAG_DIM_BEHIND);//此行代码主要是解决在华为手机上半透明效果无效的bug
