...
public class SettingsResetPinActivity extends BaseActivity implements
		OnClickListener {
	...
	private void initView() {
		...
		// 打开软键盘
		InputMethodManager imm = (InputMethodManager) SettingsResetPinActivity.this
				.getSystemService(SettingsResetPinActivity.this.INPUT_METHOD_SERVICE);
	...
	private boolean judgePassWord() {
		for (int i = 0; i < 4; i++) {
			if (!String.valueOf(pw.charAt(i)).equals(
		...
		public void handleMessage(android.os.Message msg) {
			...
			if (!scanEditTextHasNull()) {// 都 已输入
				if (judgePassWord()) {
					...
					ToastUtil.showToast(SettingsResetPinActivity.this, "PIN正确");
				} else {

					ToastUtil.showToast(SettingsResetPinActivity.this, "PIN错误");
				}
			} else {
	...
	public void onClick(View v) {
		...
		switch (v.getId()) {
		case R.id.pin_reset:
			Intent intent = new Intent(SettingsResetPinActivity.this,
					SettingsResetPinActivity2.class);
