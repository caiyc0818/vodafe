...
public class BuyActivity extends BaseActivity implements RequestListener {
	...
	private void initView() {
		...
		for (int i = 0; i < plansDetail.getLevelList().size(); i++) {
			if ("100000".equals(plansDetail.getLevelList().get(i).getLevelId())) {
				tv_center.setText(getString(R.string.settings_do_you) + " " + "300MB "
						+ getString(R.string.settings_bundle) + "?");
				dataId = plansDetail.getLevelList().get(i).getLevelId();
			} else if ("100001".equals(plansDetail.getLevelList().get(i).getLevelId())) {
				tv_center.setText(getString(R.string.settings_do_you) + " " + "500MB "
						+ getString(R.string.settings_bundle) + "?");
				dataId = plansDetail.getLevelList().get(i).getLevelId();
			} else if ("100002".equals(plansDetail.getLevelList().get(i).getLevelId())) {
				tv_center.setText(getString(R.string.settings_do_you) + " " + "800MB "
						+ getString(R.string.settings_bundle) + "?");
				dataId = plansDetail.getLevelList().get(i).getLevelId();
			}

			if ("100006".equals(plansDetail.getLevelList().get(i).getLevelId())) {
				callId = plansDetail.getLevelList().get(i).getLevelId();
			} else if ("100007".equals(plansDetail.getLevelList().get(i).getLevelId())) {
				callId = plansDetail.getLevelList().get(i).getLevelId();
			} else if ("100008".equals(plansDetail.getLevelList().get(i).getLevelId())) {
				callId = plansDetail.getLevelList().get(i).getLevelId();
			}
		}
	...
	public void onClick(View v) {
		...
		switch (v.getId()) {
		case R.id.tv_yes:
			IRequest.post(5, URLs.ADDOPTIONAL, RequestJSon.Optionaloffering(dataId, callId,true), BuyActivity.this);
