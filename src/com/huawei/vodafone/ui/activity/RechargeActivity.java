...
public class RechargeActivity extends BaseActivity implements OnClickListener {
	...
	private void initView() {
		...
		fake();
		if (getIntent().getIntExtra("number", 0) == 10) {
			dataselect(2);
		}
	...
	public void onclickYes() {
		...
		dialog.show();
		IRequest.post(URLs.RECHARGE, 19,
	...
	private void verifyProceedEnabled(){
		boolean condition1=select == 1||select == 2||select == 3||(select == 4 && !StringUtils.isEmpty(amount_data
				.getText().toString()));
		String type=paymentPop.getText().toString();
		boolean condition2=type.equals(payType[0])&&!StringUtils.isEmpty(visa_debit_number.getText().toString())
