...
public class PushReceiver extends BroadcastReceiver {
	...
	private void showwhat(Context context, String data) {
		...
		try {
			...
			Parse(context, json.optString("channel"), json.optString("content"));
	...
	private void Parse(Context context, String channel, String content) {
		...
		mBuilder.setContentTitle("Smart Pricing");
		...
		if (AndroidUtils.getModel().equals("EVA-AL00")) {
			if (!AndroidUtils.isAppOnForeground()) {
				...
				showdialog(context, channel, content);
			} else {
				if (receiver == null) {
				...
				receiver.onClicked(channel, content);
			}
		} else {
	...
	private void showdialog(final Context context, final String channel,
		...
		if (!dialog.isShowing()) {
			...
			case "DataUsgNtfctnThrshld1":
				dialog.setMessage(content, "Want more data?", "Yes, please",
						"No, thanks");
				break;
			case "RchrgReminder":
			case "Content Bundle":
			case "UpSell Bundle":
			case "LowBalance":
				dialog.setMessage(content, "We've noticed...", "Yes, please",
						"No, thanks");
				break;
			default:
				break;
